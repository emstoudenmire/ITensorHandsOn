...
function TuckerDecomp(A::ITensor; algorithm="naive_tucker", cutoff=nothing, solver=nothing) 
  return computeHOSVD(Algorithm(algorithm), A; solver)
end
...
function computeHOSVD(::Algorithm"naive_tucker", A::ITensor; cutoff, solver)
  Core = copy(A)
  Factors = Vector{ITensor}([])
  for i in inds(A)
...
function computeHOSVD(::Algorithm"smarter_tucker", A::ITensor; cutoff, solver)
  Core = copy(A)
  Factors = Vector{ITensor}([])
...
function computeHOSVD(::Algorithm"truncated_tucker", A::ITensor; cutoff, solver)
...
println("Computing Tucker decomposition")
