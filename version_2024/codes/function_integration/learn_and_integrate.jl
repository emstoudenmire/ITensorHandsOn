...
function integrate(M::MPS)
  ...
  for (m, s) in zip(M, siteinds(M))
    # Task TODO: implement the contraction of the "I" tensor
    # with vectors [1/2, 1/2] represented as ITensors on 
    # each site index s. Contracting each MPS tensor "m"
    # with these vectors performs the definite integral ∫₀¹f(x)
    # for a function f(x) represented by the MPS M.
